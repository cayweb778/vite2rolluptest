...
async function generateCode() {
  let startDate = dynamicTenant.value.target.startDate
  let date1: any = formatTimer(dayjs(startDate).subtract(1, "month"))

  // 获取最新
  return await useRouteApi(findBillCode2, {schemaName: dynamicTenantId})({
    type: "QC",iyear: dynamicYear.value,time:date1.substring(0,date1.length-2)
  })
}
...
const startReview = async (b) => {
  ...
  if(!b){
    ...
    if(!hasBlank(stockWareData.value.hxIsum )&&parseFloat(stockWareData.value.hxIsum) !=0){
      ...
      if(!hasBlank(stockWareData.value.hzhcNum)&&parseFloat(stockWareData.value.hzhcNum)!=0){
      loadMark.value=false
...
const focusNext =  async (r, c) => {
  ...
  // 查找下一个
  let list = getDataSource();
  let filters:any = ['isGive','cinvodeType','cinvodeName','cunitid','price','icost','itaxprice','baseQuantity']
  // 要求填批号才填写
...
async function setCGRKD_DATA() {
  ...
  if((parseFloat(baseQuantityTotal)-parseFloat(isumRukuTotal))-parseFloat(isumRukuTotal)==0){

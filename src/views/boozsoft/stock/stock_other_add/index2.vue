...
const tableAdd = () => {
  if(stockWareData.value.bstyle!=='其他入库'){
    return message.error('由其他单据生成,不能进行插行！')
  }
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if(stockWareData.value.bstyle!=='其他入库'){
      loadMark.value=false
      message.error('当前单据由'+stockWareData.value.bstyle+'业务生成，不能直接删除，请通过'+stockWareData.value.bstyle+'业务单据进行处理')
      return false
    }
...
const startReview = async (b) => {
  ...
  // else if(formItems.value.bstyle=='其他入库单' && !b){
    ...
    // 可用量不足 弹出框提示
    let currData2=await useRouteApi(verifyStockXCLList, { schemaName: dynamicTenantId })({queryType:'keyong',list:JSON.stringify(verifylist),rkBcheck:dynamicTenant.value.target?.kcCgrkCheck,ckBcheck:dynamicTenant.value.target?.kcXsckCheck,bdocumStyle:titleValue.value,iyear:dynamicYear.value})
    // 如果是负数强制转换成正数比较
    ...
    if(currData2.length>0){
      return openLackPage(true,{data:currData2,queryType:'keyong',dynamicTenantId:dynamicTenantId.value})
    }
  // }
    ...
    if(formItems.value.bstyle=='借入入库'){
      for (let i = 0; i < list.length; i++) {
        if(!hasBlank(list[i].sourcecode)){
          ...
          let baseQuantitySum:any=list.filter(t=>t.cinvode==list[i].cinvode).map(tx=>parseFloat(tx.baseQuantity)).reduce((n,m)=>n+m)||0
          ...
          let resouceResidueBaseQuantity:any=parseFloat(resouceBaseQuantitySum)-parseFloat(resouceRukuSum)
          ...
          let icostSum:any=list.filter(t=>t.cinvode==list[i].cinvode).map(tx=>parseFloat(tx.icost)).reduce((n,m)=>n+m)||0

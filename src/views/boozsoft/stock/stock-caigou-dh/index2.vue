    ...
    createConfirm({
      ...
      onOk: async () => {
        ...
        if(canzhao.value){
          for (let i = 0; i < dataList.length; i++) {
            // 删除下游单据
            await useRouteApi(delXySourceCcodeAndXyBillStyle, {schemaName: dynamicTenantId})({xyBillStyle:'CGDHD',ccode:dataList[i].sourcecode})
            // 上游单据明细
...
const startReview = async (b) => {
  ...
  if(b){
    ...
    // 来源单据是入库单--生成红蓝
    else if((res.sourcetype=='CGRKD' || res.sourcetype=='QT')){
      ...
      if(dynamicTenant.value.target?.kcEstimated=='1'){
        ...
        if(parseFloat(sourcedate)<parseFloat(ddate)){
          ...
          for (let i = 0; i < sourcecodeList.length; i++) {
            ...
            sourceData.id=null
            sourceData.ddate=formItems.value.ddate
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            if(dynamicTenant.value.target?.kcEstimated=='1'){
              ...
              sourceDataList.ddate=formItems.value.ddate
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        jiesuans.cgUnitId=list[i].unitList[0]?.value
...
const tableDataChange =  async (r,c) => {
  ...
  switch (c) {
    case 'cgUnitId':
      ...
      break;
    case 'cnumber':
      let o:any = assetsCardList.value.filter(it => tempType.value=='one'?(it.stockNum == r.cinvode) :tempType.value=='three'? (it.stockBarcode == r.bcheck1 || it.stockUnitBarcode1 == r.bcheck1 || it.stockUnitBarcode2 == r.bcheck1) : (it.stockName == r.cinvodeName))[0]
      r.cinvodeInfo = o
    ...
    case 'taxprice':// 含税单价
      if (!hasBlank(r.cnumber)) {
        ...
        // 无税金额=价税合计÷税率
        r.icost =parseFloat(String(r.isum/itaxrate)).toFixed(4)
        // 无税单价
        r.price =parseFloat(String(r.icost/n)).toFixed(10)
        // 税额=价税合计-无税金额
...
async function findBySupPrice(data) {
  ...
  let memberprice:any=supPrice!==''?supPrice.memberprice:0
  ...
  if(supPrice!==''){
    ...
    if(dynamicTenant.value.target.cgPriceIsRate=='1'){
      ...
      if(parseFloat(data['tempItaxrate'])!==0){
  ...
  data['taxprice']=hasBlank(data['taxprice'])?memberprice:data['taxprice']
  data['tempTaxprice']=data['taxprice']
  await tableDataChange(data,'taxprice')
}
...
const barCodeThrowData = async (map) => {
  ...
  let list=getDataSource().filter(t=>!hasBlank(t.cinvode)&&t.cgUnitId==stockUnitId&&(barCodeType=='1'?t.bcheck1==barCodeVal:t.cinvode==barCodeVal))
  if(list.length==0){
    let temp:any={
    ...
    list[0].cnumber=parseFloat(list[0].cnumber)+1

      ...
      <div>
        ...
        <div>
          <a-select v-model:value="selectSearchValue" class="acttdrd-search-select" style="width: 150px;text-align: left;font-size: 12px;"
            ...
            <a-select-option value="activityCode">活动编码</a-select-option>
            <a-select-option value="activityName">活动名称</a-select-option>
          </a-select>
          <a-input-search placeholder="" v-model:value="inputsearchtext"
                          class="acttdrd-search-input" @search="selectSearch" style="width: 150px;text-align: left;font-size: 12px;"/>
...
const selectSearchValue = ref('activityCode');
...
async function selectSearch() {
  let all = tableData.value
  if (selectSearchValue.value === 'activityCode') {
    tableData.value = all.filter((a) => !hasBlank(a.activityCode) &&a.activityCode.indexOf(inputsearchtext.value) !== -1);
  }
  if (selectSearchValue.value === 'activityName') {
    tableData.value = all.filter((a) => !hasBlank(a.activityName) && a.activityName.indexOf(inputsearchtext.value) !== -1);
  }

...
async function contentSwitch(action) {
  if (action == 'curr' && !hasBlank(formItems.value.ccode) && await operateBefore([{ccode: formItems.value.ccode,bcheck:formItems.value.bcheck}]))return false
  loadMark.value = true
  await flushPeriod()
  let res = await useRouteApi(findBillByCondition, {schemaName: dynamicTenantId})({
    ...
    action: action,
    curr: routeData?.ccode != null?routeData?.ccode:( formFuns.value.getFormValue()?.ccode || ''),
    bdocum: '0',
  })
  let ckNumber = 0;
  let thNumber = 0;
  ...
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if(hasBlank(taskData.length)){
      ...
      if(taskData[0].caozuoUnique!==useUserStoreWidthOut().getUserInfo.id){
        ...
        //await useRouteApi(stockBalanceTaskEditNewTime, { schemaName: dynamicTenantId })(taskData[0].id)
        taskIds.value.push(taskData[0].id)
        execDel()
      }
    }
  }
  ...
  if ('' == err && !biandong.value && ((!(status.value == 1 &&  !hasBlank(list[0].sourcecode) && list[0].sourcetype == 'XSCKD'))) || (status.value == 2)){
    ...
    let setParm = [...new Set(list.map(it=>(it.cwhcode1+'='+it.cinvode+'='+it.batchId+'='+it.dpdate+'='+it.dvdate)))]
     ...
     for (let s of setParm) {
       ...
       let oNum = 0;tList.map(it=> oNum+=parseFloat(it?.baseZOld || '0' ))
       let kNum = 0; // 可用量
       if (checkParm.xsOutXhd){ // 是否允许超量
         ...
         if (!checkParm.isOut){ // 如果退货单跳过
           ...
           if (tStock["stockPropertyBatch"] == '1'){
             if (r == null || (tNum >oNum &&  parseFloat(r) < tNum)){
               let lj = 0
               tList.map(it2=>{
                 let z = {baseQuantity:0}
                 if (r != null){ // 当前现存量不存在
                   let bn = (parseFloat(r) -lj) < 0?0: (parseFloat(r) -lj)
                   z.baseQuantity =it2?.xsUnitId == it2?.cunitid?bn: bn / parseFloat((it2?.unitList[it2?.unitList.findIndex(it => it.id == it2?.xsUnitId)]?.conversionRate) || 0)
                 }
                 ...
                 icList.push({lineId:it2?.lineId, stockNum:s,stockName:tStock.stockName,stockGgxh:tStock.stockGgxh,unitName:cunitFormat(it2?.unitList,it2?.xsUnitId),baseQuantity:it2.quantity,lackBaseQuantity:Math.abs(ce>0?0:ce),batchId:it2.batchId})
             ...
             kNum = parseFloat(r)
             // 可用量 > 消单数量
             if (tNum > oNum && kNum < tNum){ // 數量改小
               let c = parseFloat((tList[0]?.unitList[tList[0]?.unitList.findIndex(it => it.id == tList[0]?.xsUnitId)]?.conversionRate) || 0)
               ...
               debugger
               icList.push({
                 lineId:tList[0]?.lineId, stockNum:s,stockName:tStock.stockName,stockGgxh:tStock.stockGgxh,unitName:cunitFormat(tList[0].unitList,tList[0].xsUnitId),baseQuantity:cn,lackBaseQuantity:Math.abs(rn-cn),batchId:tList[0].batchId
               })
             }
           }
         }
...
async function stockXclCheck(list) {
  ...
  try {
    ...
    for (let s of setParm) {
      ...
      let tList = list.filter(it=>it.cwhcode1 == parms[0] &&  it.cinvode ==  parms[1] && (isB?(it.batchId ==  parms[2] && it.dpdate ==  parms[3] && it.dvdate ==  parms[4]):true))
      let tNum = 0;tList.map(it=> tNum+=parseFloat(it.baseQuantity))
      let oNum = 0;tList.map(it=> oNum+=parseFloat(it.baseZOld))
      let kNum = 0; // 可用量
      if (checkParm.xsOutXhd){ // 是否允许超量
          ...
          if (tStock["stockPropertyBatch"] == '1'){
            if (r == null ||(tNum >oNum &&  parseFloat(r) < tNum)){
              ...
              tList.map(it2=>{
                ...
                let ce = z?.baseQuantity-parseFloat(it2.quantity)
                err+=`仓储位置【${it2.cwhcodeText}】下存货编码【${it2.cinvode}】的存货${tList.length>1?'多行合计总':''}现存量不足,现存量数量【${z.baseQuantity}】,差额数量【${Math.abs(ce>0?0:ce)}】;`
...
const chChange = async(record) => {
  ...
  getKylNumber(record)
  // 获取价格
  await getCurrStockPrice(record)
}
...
const openCodePage = () => {
  ...
  if(formFuns.value.getFormValue().cwhcode==undefined){
...
const barCodeThrowData = async (map) => {
  ...
  chFocus.value=barCodeType=='2'?'one':'three'
  let oldList=getDataSource()
  let list=getDataSource().filter(t=>!hasBlank(t.cinvode)&&t.xsUnitId==stockUnitId&&(barCodeType=='1'?t.cinvodeBarcode==barCodeVal:t.cinvode==barCodeVal))
  if(list.length==0){
    let temp:any={
      cinvode:data.stockNum,
      quantity:'1',
      cinvodeBarcode:barCodeType=='1'?barCodeVal:data.stockBarcode,
      itaxrate:formItems.value.rate,
      tempItaxrate:formItems.value.rate,
    }
    let h =formFuns.value.getFormValue()
    if (hasBlank(temp.cwhcode) && !hasBlank(h.cwhcode)){
      let cangkuInfo =  ckListOptions.value.filter(it => it.id == h.cwhcode)[0]
      if (null != cangkuInfo) {
        temp['cangkuDuli'] = cangkuInfo.ckIsDuli
        if (cangkuInfo.ckIsDuli == '1') {
          temp['cwhcode'] = h.cwhcode
          temp['cwhcodeText'] = cangkuInfo.ckName
        }
      }
    }
   await chChange(temp)
    temp.xsUnitId=stockUnitId
    temp.tempCunit=stockUnitId
    temp.cinvodeBarcode=barCodeType=='1'?barCodeVal:data.stockBarcode
    tableDataChange(temp,'quantity')
    list.push(temp)
  }
  else{
    // await findBySupPrice(list[0])
    list[0].quantity=parseFloat(list[0].quantity)+1
    tableDataChange(list[0],'quantity')
  }
...
const operateBefore = async (rows) => {
  ...
  if (code != 0){
    if (code == 1){
      createWarningModal({title: '温馨提示', content: `单据已不存在，请刷新当前单据！`})
      routeData.ccode = null
      routeData.type = null
      formItems.value = {}
      formFuns.value.setFormValue({})
      ...
      createWarningModal({title: '温馨提示', content: `单据已发生变化，重新刷新当前单据！`})
      return false
    }
  }
  return false

  ...
  <div class="app-container">
    <div class="app-container-top lcr-theme-div" style="background-color: rgb(41 150 199)">
      ...
      <div>
        ...
        <div>
          <div class="acttd-right-d-search"  v-if="status==3">
            <InputSearch
...
import {
  ...
  SyncOutlined,
  VerticalLeftOutlined,BarcodeOutlined,
...
async function verifyRowXCLData(r) {
  if(hasBlank(r.cinvode)){
    message.error("请先选择存货！")
    return
  }
  if(hasBlank(r.cwhcode)){
...
const barCodeThrowData = async (map) => {
  loadMark.value=true
  if(hasBlank(formItems.value.cwhcode)){
    message.error("请先选择调出仓库后扫码！")
    return
  }
  ...
  if(list.length==0){
    ...
    list[0].tempCnumber=parseFloat(list[0].tempCnumber)+1

      ...
      <div class="acb-centent">
        ...
        >
          ...
          <template #batchId="{ record }">
            <template v-if="!canzhao&&record?.editTwelve">
              <span v-if="titleValue==0">
          ...
          <template #dpdate="{ record }">
            <template v-if="!canzhao&&titleValue==0&&record?.editDpdate">
          ...
          <template #dvdate="{ record }">
            <template v-if="!canzhao&&titleValue==0&&record?.editFifteen">
...
import BarCode from "/@/views/boozsoft/stock/stock-caigou-dh/popup/barCode.vue";
import {
  BarcodeOutlined,
  CheckOutlined,
  CopyOutlined,
...
import {
  ...
  delXySourceCcodeAndXyBillStyle,
  findBillByCondition,
  findBillCode,
...
const advancing = (e) => {
  return null == e?{}:JsonTool.parseProxy(e)
}
const routeData:any = advancing(route?.query);
      ...
      onOk: async () => {
        ...
        if(canzhao.value){
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            sourceData.isumRuku=parseFloat(sourceData.isumRuku)==0?'0':parseFloat(sourceData.isumRuku)-(parseFloat(list[i].baseQuantity))
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id))  {
    ...
    if(b&&canzhao.value && (res.sourcetype=='CGDHD' || res.sourcetype=='QC')&&(dynamicTenant.value.target.apSourceFlag=='0' || hasBlank(dynamicTenant.value.target.apSourceFlag))){
      ...
      for (let i = 0; i < listdata.length; i++) {
        ...
        jiesuans.cgUnitId=listdata[i].unitList[0]?.value
        ...
        // console.log('来源单据无税单价='+sourceData.price)
        // console.log('来源单据计量单位换算率='+sourceUnitRate)
        // console.log('入库单据计量单位换算率='+unitRate)
        ...
        tableDataChange(listdata[i],'price')
      }
      let jieSuanData:any={}
...
async function saveData() {
  ...
  if (saveCheck(list)) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // 获取来源单据
      let sourceData=await useRouteApi(findByStockWaresCcodeAndLineCode, {schemaName: dynamicTenantId})({ccode:o.sourcecode,lineCode:o.sourcedetailId})
      console.log(sourceData)
      if(parseFloat(o.baseQuantity)>parseFloat(sourceData.baseQuantity)){
        return message.error('不能超出参照单据主数量！')
      }
    }
...
const tableDataChange =  async (r,c) => {
  ...
  if(isNaN(r.itaxrate)){
    r.itaxrate=null
  }else if(!hasBlank(dynamicTenant.value.target.cgRate)){
    r.itaxrate=dynamicTenant.value.target.cgRate
  }
  switch (c) {
    case 'cnumber':
...
const findByUnitList = async (record) => {
  let o:any = assetsCardList.value.filter(it => tempType.value=='one'?(it.stockNum == record.cinvode) :tempType.value=='three'? (it.stockBarcode == record.bcheck1 ||it.stockUnitBarcode1 == record.bcheck1 || it.stockUnitBarcode2 == record.bcheck1) : (it.stockName == record.cinvodeName))[0]
  record.unitList=[]
...
const focusNext =  async (r, c) => {
  ...
  // 查找下一个
  let list = getDataSource();
  let filters:any = ['bcheck1','isGive','cinvodeType','cinvodeName','itaxprice','baseQuantity','cunitid','showNumber','taxprice','isum','itaxrate']
  // 要求填批号才填写
...
async function setCGDHD_data() {
  ...
  let cgRate=hasBlank(dynamicTenant.value.target.cgRate)?1:1+(parseFloat(dynamicTenant.value.target.cgRate)/100)
  console.log(cgRate)
  // 日期是否已结账
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    // 计算主计量....
    slChange0(temp)
    tableDataChange(temp,'price')

    temp.id=null
    temp.rate=hasBlank(dynamicTenant.value.target.cgRate)?'0':dynamicTenant.value.target.cgRate
    temp.bdocumStyle='0'
  ...
  createConfirm({
    ...
    onOk: async () => {
      ...
      formItems.value.rate=hasBlank(dynamicTenant.value.target.cgRate)?'0':dynamicTenant.value.target.cgRate
...
const barCodeThrowData = async (map) => {
  loadMark.value=true
  let data=map.stockData
  let barCodeVal=map.barCodeVal
  let barCodeType=map.barCodeType
  let stockUnitId=map.stockUnitId
  tempType.value=barCodeType=='2'?'one':'three'
  let oldList=getDataSource()
  let list:any=getDataSource().filter(t=>!hasBlank(t.cinvode)&&t.cgUnitId==stockUnitId&&(barCodeType=='1'?t.bcheck1==barCodeVal:t.cinvode==barCodeVal))
  if(list.length==0){
    let temp:any={
      cinvode:data.stockNum,
      cnumber:titleValue.value==0?1:-1,
      tempCnumber:titleValue.value==0?1:-1,
      bcheck1:barCodeType=='1'?barCodeVal:data.stockBarcode,
      itaxrate:formItems.value.rate,
      tempItaxrate:formItems.value.rate,
      cwhcode:formItems.value.cwhcode,
    }
    findByUnitList(temp)
    // 校验现存量
    if(titleValue.value==1){ verifyRowXCLData(temp) }

    temp.cgUnitId=stockUnitId
    temp.tempCgUnitId=stockUnitId
    temp.bcheck1=barCodeType=='1'?barCodeVal:data.stockBarcode,
    tableDataChange(temp,'cnumber')
    tableDataChange(temp,'price')
    list.push(temp)
  }
  else{
    // 校验现存量
    if(titleValue.value==1){ verifyRowXCLData(list[0]) }
    list[0].tempCnumber=titleValue.value==0?parseFloat(list[0].tempCnumber)+1:(parseFloat(Math.abs(list[0].tempCnumber))+1)*(-1)
    list[0].cnumber=list[0].tempCnumber
    tableDataChange(list[0],'cnumber')
    tableDataChange(list[0],'price')
  }
  let newlist=Array.from(new Set([...list,...oldList]));
  setTimeout(()=>{
    loadMark.value=false

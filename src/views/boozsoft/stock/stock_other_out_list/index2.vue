...
const startDel = async () => {
  ...
  if (checkRow.value.filter(it=>it.bstyle != '其他出库单').length != 0){
    createWarningModal({title: '温馨提示', content: `选中的单据中存在收发方式非其他出库的单据，不能直接删除！`})
    return  false
  }
  ...
  if ((checkRow.value.filter(it => it.bcheck == '1').length > 0)) {
    ...
    await useRouteApi(delBatch, {schemaName: dynamicTenantId})({codes: JsonTool.json([...new Set(checkRow.value.map(it => it.ccode))]),type: 'XSCKD'})

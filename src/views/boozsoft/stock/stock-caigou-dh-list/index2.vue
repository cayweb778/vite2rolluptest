...
async function delFun() {
  let list=dataType.value=='0'?getSelectRows1():getSelectRowsMX()
  if(list.length==0){
    return message.error('提示：至少选择一项进行删除！！')
  }
  // 结账操作
  ...
  createConfirm({
    ...
    onOk: async () => {
      ...
      for (let i = 0; i < list.length; i++) {
        // 上游单据 && 是否生成的到货单
        if(list[i].sourcetype!==undefined&&!hasBlank(list[i].sourcetype)){
          // 删除下游单据
...
const reloadPageMX = async () => {
  ...
  let mxlist=await useRouteApi(findAllStockWarehTongjiMX_CGDHD,{schemaName: dynamicTenantId})(popData.value)
  // 金额格式化
  for (let i = 0; i < mxlist.length; i++) {
    if(!hasBlank(mxlist[i].cwhcode)){
      let cangku=await getCKName(mxlist[i].cwhcode)
      mxlist[i].cwhcodeName=cangku
    }
  }

  tableDataAll.value=mxlist
  setTimeout(()=>{
      paginationNumber.value=mxlist.length
      showPaginationText.value=true
      calculateTotalMX(mxlist)
      tableData.value =replenishTrs(mxlist)
      loadMark.value = false
  },300)

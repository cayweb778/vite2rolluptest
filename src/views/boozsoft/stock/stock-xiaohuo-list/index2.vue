    ...
    <div class="app-container">
        ...
        >
          ...
          <template #ccode="{ record }">
            <a style="color: blue;" @click="toRouter(record,'list')">{{record.ccode}}</a>
          </template>
...
const startDel = async () => {
  ...
  if ((checkRow.value.filter(it => it.bcheck == '1').length > 0)) {
    createWarningModal({title: '温馨提示', content: `选中的单据中存在已审核单据，请先排除！`})
     ...
     await useRouteApi(delBatch, {schemaName: dynamicTenantId})({codes: JsonTool.json([...new Set(checkRow.value.map(it => it.ccode))]),type: 'XHD'})
...
async function toRouter(data,type) {
  if (type=='list' && await operateBefore([data]))return false

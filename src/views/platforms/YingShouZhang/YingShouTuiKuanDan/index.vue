...
async function cvencodeChange(){
  ...
  if (formItems.value.cvencode!=''&&formItems.value.cvencode!=null){
    ...
    if (arSourceFlag.value!='1') {
      qcysdList.value = qcList.filter(item => item.busStyle == 'YSD')
    } else {
      qcysdList.value = qcList.filter(item => item.arStyle == 'YSD')
    }
    ...
    if (arSourceFlag.value=='1') {
      tableData1.value.push(...saleousingList.value.filter(item => item.busStyle !='XHD' && item.busStyle !='QCXHD' && item.bworkable=='1').map(item=>{
      ...
      if(arCheckFlag=='1'){
        tableData1.value.push(...saleousingList.value.filter(item => item.busStyle != 'XSFP' && item.busStyle != 'QCXSFP' && item.bworkable=='1').map(item=>{
          item.hxStyle = item.billStyle
          item.hxCcode = item.ccode
          if (item.hxIsum!=null && item.hxIsum !='') {
            item.whxIsum = sub(item.isum, item.hxIsum)
        ...
        tableData1.value.push(...saleousingList.value.filter(item => item.busStyle != 'XSFP' && item.busStyle != 'QCXSFP').map(item=>{
          item.hxStyle = item.billStyle
          item.hxCcode = item.ccode
          if (item.hxIsum!=null && item.hxIsum !='') {
            item.whxIsum = sub(item.isum, item.hxIsum)
...
async function saveData() {
  ...
  if (getDataSource().length>0){
    for (let i = 0; i < getDataSource().length; i++) {
      ...
      if (!hasBlank(item.cunitid) && !hasBlank(item.isum)) {
        a++
      }
      if (!hasBlank(item.cunitid) && hasBlank(item.isum)) {
        createErrorModal({
...
function countFentan(){
  if(tableData.value.filter(item=>item.isum!=null&&item.isum!='').length==0){
    createErrorModal({
      iconType: 'warning',
      title: '温馨提示',
      content: '退款明细不能为空！'
  ...
  let num1 = isum.value
  let num2 = formItems.value.idiscount
  tableData1.value.forEach(item=>{
    if(item.isum>0){
      item.hxMoney = item.whxIsum
      item.tempOne = item.hxMoney
      num1 = sub(num1,item.whxIsum)
    }
  })
  if(num2!=null && num2!='') {
    tableData1.value.forEach(item => {
  ...
  tableData1.value.forEach(item => {
    if (num1 < 0) {
      if (item.isum < 0) {
        let whxIsum = sub(item.whxIsum,item.idiscount==null?'0':item.idiscount)
        if (whxIsum <= num1) {
          item.hxMoney = whxIsum
          item.tempOne = item.hxMoney

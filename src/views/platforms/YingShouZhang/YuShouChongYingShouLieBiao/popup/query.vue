...
const openHeadSelectContent = async (type) => {
  ...
  switch (type) {
    case 'cdepcode':
      const result = await useNcModalData(To.DEPT, {dynamicTenantId: databaseTrue.value})
      ...
      break;
    case 'cpersoncode':
      const result2 = await useNcModalData(To.PSN, {dynamicTenantId: databaseTrue.value})

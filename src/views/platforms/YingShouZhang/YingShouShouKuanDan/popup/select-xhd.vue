...
async function reloadList(){
  ...
  if (arSourceFlag.value=='1') {
    tableDataAll.value.push(...xhdList.value.filter(item => item.busStyle !='XHD' && item.busStyle !='QCXHD' && item.bworkable=='1').map(item=>{
    ...
    if(arCheckFlag=='1'){
      tableDataAll.value.push(...xhdList.value.filter(item => item.busStyle != 'XSFP' && item.busStyle != 'QCXSFP' && item.bworkable=='1').map(item=>{
        item.hxStyle = item.billStyle
        item.hxCcode = item.ccode
        if (item.hxIsum!=null && item.hxIsum !='') {
          item.whxIsum = sub(item.isum, item.hxIsum)
      ...
      tableDataAll.value.push(...xhdList.value.filter(item => item.busStyle != 'XSFP' && item.busStyle != 'QCXSFP').map(item=>{
        item.hxStyle = item.billStyle
        item.hxCcode = item.ccode
        if (item.hxIsum!=null && item.hxIsum !='') {
          item.whxIsum = sub(item.isum, item.hxIsum)

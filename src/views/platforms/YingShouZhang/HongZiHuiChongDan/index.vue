...
import {
  ...
  findByTypeList, findWhxskd, findXyCsourceByCcodeAndXyccode, saveSaleousing, saveStockXyCsource,
  saveWriteOffList,
  saveYsyf,
  saveYsyfsList
...
async function cvencodeChange(){
  ...
  if (formItems.value.cvencode!=''&&formItems.value.cvencode!=null){
    ...
    const qcysdList = qcList.filter(item=>item.busStyle=='YSD')
    if(formItems.value.busStyle=='SKD') {
      ...
      tableData.value = qcskdList.filter(item => item.arIsumBenbi > 0).map(item => {
      ...
      tableData1.value = qcskdList.filter(item => item.arIsumBenbi < 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
        item.isum = item.arIsumBenbi
      ...
      tableData2.value = qcysdList.filter(item => item.ysIsumBenbi > 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
      ...
      tableData3.value = qcysdList.filter(item => item.ysIsumBenbi < 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
        item.isum = item.ysIsumBenbi
          ...
          if (formItems.value.busStyle=='SKD') {
            ...
            for (let j = 0; j < list.length; j++) {
              ...
              if(item1.sourcetype=='QCYSD'){
                ...
                const saleousing = await useRouteApi(findSaleousingByCcode, {schemaName: dynamicTenantId})({
...
function fentan() {
  if(formItems.value.busStyle=='SKD'){//收款单对冲

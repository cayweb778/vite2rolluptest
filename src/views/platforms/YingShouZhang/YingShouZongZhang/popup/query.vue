...
const openHeadSelectContent =async (type) => {
  ...
  switch (type) {
    case 'cvencode':
      const result3 = await useNcModalData(To.CUSTOM,{
        datasource:{
          database: dynamicTenantId.value,
        },
        accId: defaultAdName.value,
      })
      let temp = result3[result3.length - 1]
      formItems.value.cvencode = temp.id
      ...
      break;
    case 'cvencodeJs':
      const result4 = await useNcModalData(To.CUSTOM,{
        datasource:{
          database: dynamicTenantId.value,
        },
        accId: defaultAdName.value,
      })
      let temp1 = result4[result4.length - 1]
      formItems.value.cvencodeJs = temp1.id
      // openMoalPopPage(true, {
      //   database: dynamicTenantId.value,
      //   accId: defaultAdName.value,
      // });
      break;
    case 'cdepcode':
      const result = await useNcModalData(To.DEPT, {dynamicTenantId: dynamicTenantId.value})
      formItems.value.cdepcode = result.id
      // openSelectDeptPage(true, {
      //   dynamicTenantId: dynamicTenantId.value
      // })
      break;
    case 'cpersoncode':
      const result2 = await useNcModalData(To.PSN, {dynamicTenantId: dynamicTenantId.value})
      formItems.value.cpersoncode=result2.id

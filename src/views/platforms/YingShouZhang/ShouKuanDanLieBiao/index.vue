...
import {add, pad, sub, toThousandFilter} from "../YingShouKuanQiChuYuE/calculation";
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        //判断是否可以红票对冲
        const qcList = await useRouteApi(findByCvencode,{schemaName: dynamicTenantId})({cvencode:item.cvencode,iyear:pageParameter.year})
        const qcskdList = qcList.filter(item=>item.busStyle=='SKD')
        const res = await useRouteApi(findWhxskd,{schemaName: dynamicTenantId})({year:pageParameter.year,cvencode:item.cvencode})
        const skdList = res
        let list:any = []
        let hzhcdsList:any = []
        if(item.bdocumStyle!='1'){//收款单
          //查询同一个客户下是否有未核销完成的退款,如果有进行红票对冲（期初和收款单）
          let qcskd = qcskdList.filter(item=>item.arIsumBenbi<0)
          let skd = skdList.filter(item=>item.bdocumStyle=='1')
          list = qcskd.map(item1 => {
            item1.sourcecode = item1.ccode
            item1.sourcedate = item1.ddate
            item1.sourcetype = 'QC' + item1.billStyle
            item1.isum = item.arIsumBenbi
            item1.whxIsum = sub(item1.isum, item1.hxIsum == null ? '0' : item1.hxIsum)
            item1.ljhxIsum = add(item1.hxIsum == null ? '0' : item1.hxIsum, item1.hxMoney == null ? '0' : item1.hxMoney)
            item1.type = 'YUE'
            return item1
          })
          list.push(...skd.map(item1 => {
            item1.sourcecode = item1.ccode
            item1.sourcedate = item1.ddate
            item1.sourcetype = item1.billStyle
            item1.whxIsum = sub(item1.isum,item1.hxIsum==null?'0':item1.hxIsum)
            item1.ljhxIsum = add(item1.hxIsum==null?'0':item1.hxIsum,item1.hxMoney==null?'0':item1.hxMoney)
            item1.type = 'SKD'
            return item1
          ...
          if (list.length>0){
            ...
            list.forEach(item1=>{
              if (num1 < 0) {
                if (item1.isum < 0) {
                  if (item1.whxIsum >= num1) {
                    item1.hxMoney = item1.whxIsum
                    item1.tempOne = item1.hxMoney
                    num1 = sub(num1, item1.hxMoney)
                    num2 = add(num,item1.hxMoney)
                  } else {
                    item1.hxMoney = num1
                    item1.tempOne = item1.hxMoney
                    num1 = sub(num1, item1.hxMoney)
                    num2 = add(num2,item1.hxMoney)
                  }
                }
              }
            })
            //保存红票对冲主表
            let hzhcd:any = {}
            hzhcd.iyear = pageParameter.year
            hzhcd.ccode = generateCode(dateTime)
            hzhcd.cmaker = useUserStoreWidthOut().getUserInfo.id
            hzhcd.billStyle = 'SKD'
            hzhcd.ddate = dateTime
            hzhcd.busStyle = 'SKD'
            hzhcd.cvencode = item.cvencode
            hzhcd.cdepcode = item.cdepcode
            hzhcd.cpersoncode = item.cpersoncode
            hzhcd.currency = item.currency
            hzhcd.writeOffStatus = item.writeOffStatus
            hzhcd.cmemo = item.cmemo
            hzhcd.idiscount = item.idiscount
            hzhcd.isum = isum.value
            hzhcd.hxIsum = -num2
            hzhcd.hxIdiscount = item.hxIdiscount
            hzhcd.citemcode = item.citemcode
            hzhcd.citemClass = item.citemClass
            await useRouteApi(saveArApHzhcd,{schemaName: dynamicTenantId})(hzhcd)
            let hzhcds:any = {}
            hzhcds.iyear = pageParameter.year
            hzhcds.ccode = hzhcd.ccode
            hzhcds.ddate = hzhcd.ddate
            hzhcds.lineCode = item.lineCode
            hzhcds.lineId = i+1
            hzhcds.bdocumStyle = '0'
            hzhcds.billStyle = hzhcd.billStyle
            hzhcds.busStyle = hzhcd.busStyle
            hzhcds.cvencode = hzhcd.cvencode
            hzhcds.citemcode = hzhcd.citemcode
            hzhcds.cunitid = hzhcd.cunitid
            hzhcds.cunitidF1 = hzhcd.cunitidF1
            hzhcds.citemClass = hzhcd.citemClass
            hzhcds.itaxCode = hzhcd.itaxCode
            hzhcds.itaxrate = hzhcd.itaxrate
            hzhcds.idiscount = hzhcd.idiscount
            hzhcds.isumYb = hzhcd.isumYb
            hzhcds.isum = hzhcd.isum
            hzhcds.hxIsum = hzhcd.hxMoney
            hzhcds.writeOffStatus = hzhcd.writeOffStatus
            hzhcds.cmemo = hzhcd.cmemo
            hzhcds.sourcetype = item.billStyle
            hzhcds.sourcecode = item.ccode
            hzhcds.sourcedetailId = hzhcd.sourcedetailId
            hzhcds.sourcedate = item.ddate
            hzhcds.pjCode = hzhcd.pjCode
            hzhcds.pjDate = hzhcd.pjDate
            hzhcds.pjType = hzhcd.pjType
            hzhcds.pjDhDate = hzhcd.pjDhDate
            hzhcds.cdepcode = hzhcd.cdepcode
            hzhcds.cpersoncode = hzhcd.cpersoncode
            hzhcdsList.push(hzhcds)
            for (let j = 0; j < list.length; j++) {
              const item1 = list[j]
              let hzhcds1: any = {}
              hzhcds1.iyear = pageParameter.year
              hzhcds1.ccode = item.ccode
              hzhcds1.ddate = item.ddate
              hzhcds1.lineCode = item1.lineCode
              hzhcds1.lineId = i+1
              hzhcds1.bdocumStyle = '1'
              hzhcds1.billStyle = item.billStyle
              hzhcds1.busStyle = item.busStyle
              hzhcds1.cvencode = item1.cvencode
              hzhcds1.citemcode = item1.citemcode
              hzhcds1.cunitid = item1.cunitid
              hzhcds1.cunitidF1 = item1.cunitidF1
              hzhcds1.citemClass = item1.citemClass
              hzhcds1.itaxCode = item1.itaxCode
              hzhcds1.itaxrate = item1.itaxrate
              hzhcds1.idiscount = item1.idiscount
              hzhcds1.isumYb = item1.isumYb
              hzhcds1.isum = item1.isum
              hzhcds1.hxIsum = item1.hxMoney
              hzhcds1.writeOffStatus = item1.writeOffStatus
              hzhcds1.cmemo = item1.cmemo
              hzhcds1.sourcetype = item1.sourcetype
              hzhcds1.sourcecode = item1.sourcecode
              hzhcds1.sourcedetailId = item1.sourcedetailId
              hzhcds1.sourcedate = item1.sourcedate
              hzhcds1.pjCode = item1.pjCode
              hzhcds1.pjDate = item1.pjDate
              hzhcds1.pjType = item1.pjType
              hzhcds1.pjDhDate = item1.pjDhDate
              hzhcds1.cdepcode = item1.cdepcode
              hzhcds1.cpersoncode = item1.cpersoncode
              hzhcdsList.push(hzhcds1)
            }
            await useRouteApi(saveArApHzhcdsList,{schemaName: dynamicTenantId})(hzhcdsList)
            //保存下游单据
            for (let j = 0; j < hzhcdsList.length; j++) {
              let item1 = hzhcdsList[j]
              let stockXyCsource: any = {}
              stockXyCsource.iyear = pageParameter.year
              stockXyCsource.xyBillStyle = 'HPDC'
              if (item1.type == 'YUE') {
                stockXyCsource.billStyle = 'YUE'
              } else {
                stockXyCsource.billStyle = item1.sourcetype
              }
              stockXyCsource.ccode = item1.sourcecode
              stockXyCsource.ccodeDate = item1.sourcedate
              stockXyCsource.xyccode = hzhcd.ccode
              stockXyCsource.xyccodeDate = hzhcd.ddate
              await useRouteApi(saveArXySource, {schemaName: dynamicTenantId})(stockXyCsource)
            }
          }
        } else {//退款单
          //查询同一个客户下是否有未核销完成的收款,如果有进行红票对冲（期初和收款单）
          let qcskd = skdList.filter(item=>item.arIsumBenbi>0)
          let skd = skdList.filter(item=>item.bdocumStyle!='1')
          list = qcskd.map(item1 => {
            item1.sourcecode = item1.ccode
            item1.sourcedate = item1.ddate
            item1.sourcetype = 'QC' + item1.billStyle
            item1.isum = item1.arIsumBenbi
            item1.whxIsum = sub(item1.isum, item1.hxIsum == null ? '0' : item1.hxIsum)
            item1.ljhxIsum = add(item1.hxIsum == null ? '0' : item1.hxIsum, item1.hxMoney == null ? '0' : item1.hxMoney)
            item1.type = 'YUE'
            return item1
        ...
        if (hzhcdsList.length>0){
          //修改收款单
          for (let j = 0; j < hzhcdsList.length; j++) {
            let item1 = hzhcdsList[j]
            item1.hxMoney = item1.hxIsum
            if(!hasBlank(item1.hxMoney) && item1.hxMoney!=0) {
              if (item1.type == 'YUE') {
                if (item1.idiscount != null && item1.idiscount != '') {
                  item1.hxIsum = add(item1.hxIsum == null ? '0' : item1.hxIsum, add(item1.hxMoney == null ? '0' : item1.hxMoney, item1.idiscount))
                } else {
                  item1.hxIsum = add(item1.hxIsum == null ? '0' : item1.hxIsum, item1.hxMoney == null ? '0' : item1.hxMoney)
                }
                if (sub(item1.isum, item1.hxIsum) == 0) {
                  item1.hxFlag = '1'
                } else {
                  item1.hxFlag = '0'
                }
                ...
                if (res.length > 0) {
                  if (!hasBlank(item1.hxMoney) && item1.hxMoney != 0) {
                    let ysyf = res[0]
                    ysyf.hxIsum = add(ysyf.hxIsum == null ? '0' : ysyf.hxIsum, item1.hxMoney)
                    ysyf.busStyle = 'PTSK'
...
import {findBukongCcode, saveArApHzhcd, saveArApHzhcdsList} from "/@/api/record/system/ArApHzhcd";

...
function fentan(){
  ...
  if(whxIsum.value==0){
    createErrorModal({
      iconType: 'warning',
      title: '温馨提示',
      content: '预付冲销明细的合计为零，不能进行冲销处理，请选择红票对冲！'
    })
    return false
  }
  if(whxIsum1.value==0){
    createErrorModal({
      iconType: 'warning',
      title: '温馨提示',
      content: '应付冲销明细的合计为零，不能进行冲销处理，请选择红票对冲！'
    })
    return false
  }
  if(cxIsum.value>0 && whxIsum.value<0){
    createErrorModal({
      iconType: 'warning',
      title: '温馨提示',
      content: '冲销金额和预付冲销明细的合计金额正负方向不一致，不能进行冲销处理，请稍后在试！'
    })
    return false
  }
  if(cxIsum.value>0 && whxIsum1.value<0){
    createErrorModal({
      iconType: 'warning',
      title: '温馨提示',
      content: '冲销金额和应付冲销明细的合计金额正负方向不一致，不能进行冲销处理，请稍后在试！'
    })
    return false
  }
  tableData.value.forEach(item=>{

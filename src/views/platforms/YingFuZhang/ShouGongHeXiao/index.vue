    ...
    <div class="app-container">
      ...
      <div style="margin-top: -25px;margin-left: 60px;">
        ...
        <div style="display: inline-block;float: left;margin-left: 12px;font-weight: bold;">
          <span style="color:#666666;">结算供应商名称：</span>{{ formatCvencode(cvencode) }}
        </div>
...
async function reloadTable(){
  ...
  if (apSourceFlag.value=='1') {
    saleousingList.value = saleList.filter(item => item.busStyle !='CGDHD' && item.busStyle !='QCCGDHD' && item.bworkable=='1')
  } else {
    if(apCheckFlag=='1'){
      saleousingList.value = saleList.filter(item => item.busStyle !='CGFP' && item.busStyle !='QCCGFP' && item.bworkable=='1')
    } else {
      saleousingList.value = saleList.filter(item => item.busStyle !='CGFP' && item.busStyle !='QCCGFP')
    }
  }

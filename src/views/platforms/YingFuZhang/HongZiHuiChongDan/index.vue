<template>
  <div class="app-container">
    ...
    <!--   <div><textarea>红票对冲的页面</textarea>复制内容快速定位页面</div>-->
...
import {
  deleteXyCsouceById,
  findArApYsyfByCcode, findWhxskd, findXyCsourceByCcodeAndXyccode, saveStockXyCsource,
  saveYsyf,
...
async function cvencodeChange(){
  ...
  if (formItems.value.cvencode!=''&&formItems.value.cvencode!=null){
    const qcList = await useRouteApi(findByCvencode,{schemaName: dynamicTenantId})({cvencode:formItems.value.cvencode,iyear:dynamicYear.value})
    const qcfkdList = qcList.filter(item=>item.busStyle=='FKD')
    const qcyfdList = qcList.filter(item=>item.busStyle=='YFD')
    if(formItems.value.busStyle=='FKD') {
      ...
      tableData.value = qcfkdList.filter(item => item.arIsumBenbi > 0).map(item => {
      ...
      tableData.value.push(...res.filter(item => item.bdocumStyle != '1').map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
      ...
      tableData1.value = qcfkdList.filter(item => item.arIsumBenbi < 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
        item.isum = item.arIsumBenbi
      ...
      tableData1.value.push(...res.filter(item => item.bdocumStyle == '1').map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = item.billStyle
        item.whxIsum = sub(item.isum,item.hxIsum==null?'0':item.hxIsum)
      ...
      tableData2.value = qcyfdList.filter(item => item.ysIsumBenbi > 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
      ...
      tableData2.value.push(saleousingList.value.filter(item=>item.bdocumStyle != '1' && item.isum!=0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
      ...
      tableData3.value = qcyfdList.filter(item => item.ysIsumBenbi < 0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.sourcetype = 'QC'+item.billStyle
        item.isum = item.ysIsumBenbi
      ...
      tableData3.value.push(saleousingList.value.filter(item=>item.bdocumStyle == '1' && item.isum!=0).map(item => {
        item.sourcecode = item.ccode
        item.sourcedate = item.ddate
        item.hxStyle = item.billStyle
        item.sourcetype = item.billStyle
  ...
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if (formItems.value.bcheck=='1') {
      ...
      createConfirm({
        ...
        onOk: async () => {
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'红票对冲',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'红票对冲',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在删除红票对冲,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'红票对冲',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
            }
          }
          ...
          if (formItems.value.busStyle=='FKD') {
            for (let j = 0; j < list.length; j++) {
              ...
              if(item1.sourcetype.substring(0,2)=='QC'){
                //期初应收单
                const xhdyue = await useRouteApi(findArBeginBalanceByCcode, {schemaName: dynamicTenantId})(item1.ccode)
                xhdyue.hxIsum = sub(xhdyue.hxIsum, add(item1.hxMoney == null ? '' : item1.hxMoney, item1.idiscount == null ? '' : item1.idiscount))
                xhdyue.hxFlag = '0'
            ...
            for (let j = 0; j < list.length; j++) {
              ...
              if (item1.sourcetype == 'QCYSD') {
                ...
                const saleousing = await useRouteApi(findWarehousingByCcode, {schemaName: dynamicTenantId})({
                  ccode: item1.sourcecode,
                  billStyle: item1.sourceType
                })
          ...
          if (formItems.value.busStyle=='FKD'){}
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'红票对冲',method:'删除',recordNum:formItems.value.ccode})
      ...
      if(!hasBlank(item.hxMoney) && item.hxMoney!=0) {
        if (item.type == 'YUE') {
          if (item.idiscount != null && item.idiscount != '') {
            item.hxIsum = add(item.hxIsum == null ? '0' : item.hxIsum, add(item.hxMoney == null ? '0' : item.hxMoney, item.idiscount))
          } else {
            item.hxIsum = add(item.hxIsum == null ? '0' : item.hxIsum, item.hxMoney == null ? '0' : item.hxMoney)
          }
          if (sub(item.isum, item.hxIsum) == 0) {
            item.hxFlag = '1'
          } else {
            item.hxFlag = '0'
          }
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if(!hasBlank(item.hxMoney) && item.hxMoney!=0) {
        if (item.type == 'YUE') {
          ...
          const saleousing = await useRouteApi(findWarehousingByCcode, {schemaName: dynamicTenantId})({
            ccode: item.sourcecode,
            billStyle: item.sourceType
          })
...
function fentan() {
  if(formItems.value.busStyle=='FKD'){//收款单对冲
    ...
    if(wjsIsum.value<-wjsIsum1.value){//按照wjsIsum分摊
    ...
    if(tableData2.value.filter(item=>item.id!=null&&item.id!='').length==0){
      createErrorModal({
        iconType: 'warning',
        title: '温馨提示',
        content: '蓝字应收单不能为空！'
    ...
    if(tableData3.value.filter(item=>item.id!=null&&item.id!='').length==0){
      createErrorModal({
        iconType: 'warning',
        title: '温馨提示',
        content: '红字应收单不能为空！'
    ...
    if(hxIsum2.value<-hxIsum3.value){//按照hxIsum2分摊
      let num1 = hxIsum2.value
      tableData2.value.forEach(item=>{
        if (num1 > 0) {
          if (item.isum > 0) {
            if (item.whxIsum <= num1) {
